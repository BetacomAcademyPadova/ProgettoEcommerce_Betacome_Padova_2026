...
{	
	@Query(value = "SELECT DISTINCT p.* FROM prodotti p " +
	        ...
	        "WHERE (:descrizione IS NULL OR LOWER(p.descrizione) LIKE LOWER(CONCAT('%', :descrizione, '%'))) " +
	        "AND (:prezzo IS NULL OR p.prezzo = :prezzo) " +
	        "AND (:sotto_categoria IS NULL OR LOWER(s.sotto_categoria) LIKE LOWER(CONCAT('%', :sotto_categoria, '%'))) " +
	        "AND (:colore IS NULL OR LOWER(d.colore) LIKE LOWER(CONCAT('%', :colore, '%'))) " +
	        "AND (:materiale IS NULL OR LOWER(d.materiale) LIKE LOWER(CONCAT('%', :materiale, '%'))) " +
